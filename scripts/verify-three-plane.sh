...
REMOTE_USER="${COFOUNDER_REMOTE_USER:-cofounder}"
REMOTE_HOST="${COFOUNDER_REMOTE_HOST:-spark.example.invalid}"
REMOTE_PORT="${COFOUNDER_REMOTE_PORT:-22}"
SSH_KEY="${COFOUNDER_SSH_KEY:-$HOME/.ssh/id_ed25519}"
REMOTE_REPO="${COFOUNDER_REMOTE_REPO:-/home/cofounder/cofounder-os}"
...
echo "--- Remote Status ---"
ssh "${SSH_ARGS[@]}" \
  '/home/cofounder/.local/bin/cofounderctl status 2>/dev/null' || { fail "Remote status failed"; VALIDATION_STATE="FAIL"; }
echo "--- Remote Health ---"
ssh "${SSH_ARGS[@]}" \
  '/home/cofounder/.local/bin/cofounderctl health 2>/dev/null' || { fail "Remote health failed"; VALIDATION_STATE="FAIL"; }
echo "--- Remote Smoke ---"
ssh "${SSH_ARGS[@]}" \
  '/home/cofounder/.local/bin/cofounderctl smoke 2>/dev/null' || { fail "Remote smoke failed"; VALIDATION_STATE="FAIL"; }
