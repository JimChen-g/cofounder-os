...
echo "--- Remote Status ---"
ssh "${SSH_ARGS[@]}" \
  '/home/cofounder/.local/bin/cofounderctl status 2>/dev/null' || fail "Remote cofounderctl status failed"
echo "--- Remote Health ---"
ssh "${SSH_ARGS[@]}" \
  '/home/cofounder/.local/bin/cofounderctl health 2>/dev/null' || fail "Remote cofounderctl health failed"
